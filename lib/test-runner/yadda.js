...
var language = parser.getLanguage(session.config.language);
...
function loadStepDefs(language) {
  ...
  if (session.config.dictionaryFile){
    var customDict = null;
    try{
      customDict = require(path.join(process.cwd(), session.config.dictionaryFile));
    }catch(e){
  ...
  glob.sync(session.config.stepsDir + "/**/*.js").forEach(function (file) {
...
afterEach(function () {
  if (session.config.capture || this.currentTest.state !== 'passed') {
