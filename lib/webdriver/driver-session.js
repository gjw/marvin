...
module.exports = {
  ...
  create: function (browser) {
    return this.execute(function(){
      driver = new webdriver.Builder().usingServer(browser.seleniumServer).withCapabilities(browser).build();
      driver.manage().timeouts().implicitlyWait(config.elementTimeout || 3000);
      if (config.screenSize && config.screenSize.x && config.screenSize.y){
        driver.manage().window().setSize(config.screenSize.x, config.screenSize.y);
      }
      else {
