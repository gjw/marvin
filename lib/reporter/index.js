...
exports = module.exports = function MarvinReporter(runner) {
  ...
  var baseResultsDir = session.config.resultsDir || 'results';
  var jsonOut = path.join(baseResultsDir, session.launchDate, process.pid.toString() + '.json');
  ...
  runner.on('fail', function (test) {
    ...
    if (scenario) {
      scenario.steps.push({
        ...
        duration: test.duration,
        error: errorStr,
        screenshot: getScrenshotPath(test.title),
        logs: logger.logs
      });
...
function getScrenshotPath(testName) {
  return session.launchDate + '/screenshots/' + testName.replace(/\W+/g, '-').toLowerCase() + '.png';
}
