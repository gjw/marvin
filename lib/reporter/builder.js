...
function mergeResults(resultsDir) {
  ...
  glob.sync(path.join(resultsDir, '*.json')).forEach(function (file) {
    ...
    var browserKey = getBrowserKey(results.browser);
...
function getBrowserKey(browser) {
  ...
  // @todo use util.format

  return platform + "-" + browser.browserName + "-" + version ; 
...
module.exports.prepareResults = function(resultsDir) {
  ...
  copyScreenshots(config.resultsDir, resultsDir);

  var data = mergeResults(resultsDir);

  // @todo dev: remove when done
  fs.writeFileSync("c:\\tmp.json", JSON.stringify(data));
